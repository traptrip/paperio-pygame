...
from helpers import is_intersect, get_random_coordinates
from config import *
...
class Game:
    ...
    def __init__(self, clients):
        ...
        for i, client in enumerate(clients):
            players.append(Player(i + 1, *next(coordinates),
                                  f'Player{i + 1}',
                                  CONSTS.PLAYER_COLORS[i]))
    ...
    def get_busy_points(self):
        ...
        return players_points | lines_poins

    @staticmethod
    def get_coordinates(clients_count):
        coords = [
            get_random_coordinates() for _ in range(clients_count)
        ]
        yield from coords
    ...
    def check_lose(self, player, players):
        is_lose = False

        if player.y < 0 + round(CONSTS.WIDTH / 2) or \
                player.x < 0 + round(CONSTS.WIDTH / 2) or \
                player.y > CONSTS.WINDOW_HEIGHT - round(CONSTS.WIDTH / 2) or \
                player.x > CONSTS.WINDOW_WIDTH - round(CONSTS.WIDTH / 2):
            is_lose = True
    ...
    def send_game_end(self):
        self.game_log.append({
            'type': 'end_game',
            'events': self.events,
            'scores': {p.id: p.score for p in self.losers + self.players}
        })
    ...
    @staticmethod
    def collision_resolution(players_to_captured):
    ...
    @staticmethod
    def get_command_wrapper(player):
